...
| 关节行程范围 | 见下方关节极限表 | 手动实测 2026-05-08 |

### 关节极限 (2026-05-08 手动实测)

| 状态 | M1 (θaL) | M2 (θbL) | M3 (θbR) | M4 (θaR) |
|------|----------|----------|----------|----------|
| 所有杆向后限位 | +1.67 | +1.61 | -1.62 | -1.54 |
| 所有杆向前限位 | -3.4 | -3.3 | +3.4 | +3.2 |
| a杆后限位 + b杆后限位 | +1.65 | +3.14 | -1.66 | -3.14 |

- M1/M4 (a杆) 行程约 **5.0 rad** (±3.4 ~ +1.67)
- M2/M3 (b杆) 行程约 **6.4 rad** (-3.3 ~ +3.14)
- 左右腿基本对称，M3/M4 符号取反（对调映射）
